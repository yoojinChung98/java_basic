...
/*
# 매개 변수 (parameter, argument): 인수 그 자체 = argument, 인수를 받는 애! = parameter
- 메서드가 반드시 매개변수를 가져야하는 것은 아님!!
...
public class MethodExample2 {
	...
	// 정수를 n개 전달하면 그 정수들의 총합을 리턴하는 메서드
	...
	// '콤마'로 나열되어 들어오는 여러 개의 값을 배열로 묶어서 내부로 전달합니다.
	static int calcNumberTotal2(int... nums) { // ... nums -> nums는 가변인수로 변해서 선언은 하나만 선언되었지만 값을 여러개 넣었을 때 묶어서 배열로 전달함!
		int total = 0;
		for(int n : nums) {
			total += n;
		}
	...
	public static void main(String[] args) {
		...
		int[] arr = {10,30,50,70,90,110,200};
