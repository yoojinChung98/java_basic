...
//		User kim = new User("김춘식",20);
//		User park = new User("박영희",20);
//		Collections.addAll(userList, hong, kim, park);
		
//		userList.add(new User("춘식이",13));
//		userList.add(new User("홍길동",27));
//		userList.add(new User("짹짹이",7));
		...
		//향상포문을 사용하면 안되는 이유
		//향상 포문 내에서 배열의 크기 수정이 발생하면, 인덱스에 변화가 생기므로, 향상포문은 다음요소를 땡겨오는데 오류가 생기므로 터짐.
		//정~ 쓰고 싶으면 break;로 크기 수정이 발생할 때 종료해주면 됨
		/*
