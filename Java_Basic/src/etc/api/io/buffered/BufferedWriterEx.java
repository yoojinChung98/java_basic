...
public class BufferedWriterEx {
	...
	public static void main(String[] args) {
        ...
        ex)
            ...
            -> 기본 OutputStream을 사용하면 한 글자씩 16번의 출력(접근)이 발생.
            -> 버퍼에 저장해 놓고 한번에 쓰면 한 번의 출력으로 끝나지 않을까요?
        */
		
		Scanner sc = new Scanner(System.in);
		FileWriter fw = null; // BufferedWriter 객체가 FileWriter을 필요로 해요!
		BufferedWriter bw = null;
		
		try {
			fw = new FileWriter("C:/MyWork/hello.txt");
			bw = new BufferedWriter(fw); // FileWriter 객체를 인자로 넣어줘야 해! bw = new BufferedWriter(new FileWriter("폴더및파일경로");); 
			...
			try {
				bw.close(); // 마지막으로 생성된 객체부터 닫아주세요(괄호가 닫히듯이)
				fw.close();
				sc.close();
			} catch (IOException e) {
				e.printStackTrace();
